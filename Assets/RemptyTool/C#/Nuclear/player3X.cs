...
public class player3X : MonoBehaviour
{
    public float x = 0.1f;
    ...
    void Update()
    {
        ...
        bool IsWalkingpwith = false;
        bool Isfrontingpwith = false;
        bool Isbackingpwith = false;
        bool Isrunningpwith = false;
        if (gameManager.hold == 1)
        {
            ...
            if (Input.GetKey(KeyCode.DownArrow) && gameManager.chance != 12)
            {
                ...
                {
                    Isfrontingpwith = false;
                }
                else
                {
                    Isfrontingpwith = true;
                    if (Input.GetKey(KeyCode.LeftShift))
                    {
                        rigid2D.AddForce(new Vector2(0, -100 * speed * 2), ForceMode2D.Force);
                    }
        ...
        else
        {
            if (IsWalkingpwith)
            {
                if (playerAni.GetInteger("Status") == 11)
                {
            ...
            {
                if (playerAni.GetInteger("Status") == 7)
                {
            ...
            if (Isfrontingpwith)
            {
                if (playerAni.GetInteger("Status") == 11 || playerAni.GetInteger("Status") == 6)
                {
            ...
            {
                if (playerAni.GetInteger("Status") == 6 && Input.GetKey(KeyCode.RightArrow) || playerAni.GetInteger("Status") == 6 && Input.GetKey(KeyCode.LeftArrow))
                {
                    playerAni.SetInteger("Status", 11);
                }
                else if (playerAni.GetInteger("Status") == 6 && Input.GetKey(KeyCode.UpArrow))
                { playerAni.SetInteger("Status", 12); }
                else if (playerAni.GetInteger("Status") == 8)
            ...
            if (Isbackingpwith)
            {
                if (playerAni.GetInteger("Status") == 11 || playerAni.GetInteger("Status") == 12)
                {
            ...
            {
                if (playerAni.GetInteger("Status") == 12 && Input.GetKey(KeyCode.RightArrow) || playerAni.GetInteger("Status") == 12 && Input.GetKey(KeyCode.LeftArrow))
                {
                    playerAni.SetInteger("Status", 11);
                }
                else if (playerAni.GetInteger("Status") == 12 && Input.GetKey(KeyCode.DownArrow))
                { playerAni.SetInteger("Status", 6); }
                else if (playerAni.GetInteger("Status") == 9)
                { playerAni.SetInteger("Status", 12); }
            }
        }
        if (Isrunning)
